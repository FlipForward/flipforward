...
const Footer = () => {
  ...
  return (
    <footer className="bg-gradient-hero border-t border-border py-12">
      <div
        ref={ref}
        className={`container mx-auto px-6 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
      >
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <p className="text-muted-foreground text-sm">© {currentYear} Flip Forward. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
